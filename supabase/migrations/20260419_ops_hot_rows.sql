-- Precomputed hot row-list snapshot for /repairs and /bespoke.
--
-- Previous pass moved stage-counts + overdue counts off the hot path.
-- The remaining TTFB cost on both operational routes is the "200 most
-- recent rows + customer join" fetch that runs on every request.
--
-- This pass snapshots that list into two new jsonb columns on
-- tenant_dashboard_stats. The refresh function writes them alongside
-- the existing aggregates, so the existing pg_cron (every 60 s) and
-- write-trigger (after() → refreshDashboardStatsAsync) machinery
-- already keeps them fresh. Zero new moving parts on the refresh side.
--
-- Scope: ONLY the default no-search view reads the snapshot. When
-- `?q=` is set, the live path runs an ILIKE that can reach past the
-- 200-row snapshot cap. Same for the rare edge case where the
-- snapshot is missing (fresh tenant) — clean fallback to the live path.

ALTER TABLE tenant_dashboard_stats
  ADD COLUMN IF NOT EXISTS repairs_hot_rows JSONB NOT NULL DEFAULT '[]',
  ADD COLUMN IF NOT EXISTS bespoke_hot_rows JSONB NOT NULL DEFAULT '[]';

-- Rewrite the refresh function to also populate the two row-list snapshots.
...
DECLARE
  ...
  -- New: hot row lists (200 most recent, ordered as the list renders).
  v_repairs_hot_rows JSONB;
  v_bespoke_hot_rows JSONB;
BEGIN
  ...
  SELECT COALESCE(SUM(GREATEST(0, amount_due)), 0) INTO v_outstanding
    FROM invoices WHERE tenant_id = p_tenant_id
      AND status IN ('partial','unpaid','overdue') AND deleted_at IS NULL;

  SELECT COUNT(*) INTO v_overdue_inv_count
    FROM invoices WHERE tenant_id = p_tenant_id
      AND status NOT IN ('paid','voided','draft','cancelled')
      AND due_date < v_today AND deleted_at IS NULL;

  SELECT COUNT(*) INTO v_active_jobs
    FROM bespoke_jobs WHERE tenant_id = p_tenant_id
      AND deleted_at IS NULL AND stage NOT IN ('completed','cancelled');

  SELECT COUNT(*) INTO v_active_repairs
    FROM repairs WHERE tenant_id = p_tenant_id
      AND deleted_at IS NULL AND stage NOT IN ('collected','cancelled');

  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]') INTO v_overdue_repairs
  FROM (
    ...
    WHERE r.tenant_id = p_tenant_id AND r.deleted_at IS NULL
      AND r.stage NOT IN ('collected','cancelled') AND r.due_date < v_today
    ORDER BY r.due_date ASC LIMIT 5
  ) t;

  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]') INTO v_low_stock
  FROM (
    SELECT DISTINCT ON (COALESCE(sku, id::TEXT)) id, name, sku, quantity, low_stock_threshold
    FROM inventory WHERE tenant_id = p_tenant_id AND status = 'active'
      AND deleted_at IS NULL AND track_quantity = true
      AND quantity <= COALESCE(low_stock_threshold, 1)
    ORDER BY COALESCE(sku, id::TEXT), quantity ASC LIMIT 10
  ) t;
  ...
  FROM (
    ...
    FROM repairs r LEFT JOIN customers c ON c.id = r.customer_id
    WHERE r.tenant_id = p_tenant_id AND r.stage = 'ready' AND r.deleted_at IS NULL LIMIT 5
  ) t;
  ...
  FROM (
    ...
    FROM bespoke_jobs j LEFT JOIN customers c ON c.id = j.customer_id
    WHERE j.tenant_id = p_tenant_id AND j.stage = 'ready' AND j.deleted_at IS NULL LIMIT 5
  ) t;
  ...
  FROM (
    SELECT j.id, j.title, j.stage, j.due_date, j.location_id, c.full_name AS customer_name
    FROM bespoke_jobs j LEFT JOIN customers c ON c.id = j.customer_id
  ...
  FROM (
    ...
    WHERE s.tenant_id = p_tenant_id ORDER BY s.created_at DESC LIMIT 5
  ) t;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('value', daily_value) ORDER BY day), '[]')
    INTO v_rev_spark
  FROM (SELECT date_trunc('day', created_at AT TIME ZONE v_tz) AS day, COALESCE(SUM(total), 0) AS daily_value
          FROM sales WHERE tenant_id = p_tenant_id AND created_at >= v_seven_days_ago
          GROUP BY 1 ORDER BY 1) g;

  SELECT COALESCE(jsonb_agg(jsonb_build_object('value', daily_value) ORDER BY day), '[]')
    INTO v_count_spark
  FROM (SELECT date_trunc('day', created_at AT TIME ZONE v_tz) AS day, COUNT(*) AS daily_value
          FROM sales WHERE tenant_id = p_tenant_id AND created_at >= v_seven_days_ago
          GROUP BY 1 ORDER BY 1) g;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'day', to_char(day, 'Dy'), 'sales', daily_count, 'revenue', daily_revenue
         ) ORDER BY day), '[]')
    INTO v_bar_data
  FROM (SELECT date_trunc('day', created_at AT TIME ZONE v_tz) AS day,
  ...
  FROM (SELECT stage, COUNT(*) AS cnt FROM repairs
          WHERE tenant_id = p_tenant_id AND deleted_at IS NULL GROUP BY stage) g;

  SELECT COALESCE(jsonb_object_agg(stage, cnt), '{}') INTO v_bespoke_stage_counts
  FROM (SELECT stage, COUNT(*) AS cnt FROM bespoke_jobs
          WHERE tenant_id = p_tenant_id AND deleted_at IS NULL GROUP BY stage) g;

  SELECT COUNT(*) INTO v_repairs_overdue
    FROM repairs WHERE tenant_id = p_tenant_id AND deleted_at IS NULL
      AND stage NOT IN ('collected','cancelled','ready') AND due_date < v_today;

  SELECT COUNT(*) INTO v_bespoke_overdue
    FROM bespoke_jobs WHERE tenant_id = p_tenant_id AND deleted_at IS NULL
      AND stage NOT IN ('completed','cancelled','ready') AND due_date < v_today;

  -- ── NEW: operational hot row snapshots ──────────────────────────────
  -- Only the exact columns the list UI needs, ordered as it renders
  -- (created_at DESC). Customer join is inlined as `customers` object
  -- so the client prop shape stays identical to the live-path shape.
  SELECT COALESCE(jsonb_agg(row_json ORDER BY created_at DESC), '[]') INTO v_repairs_hot_rows
  FROM (
  ...
  SELECT COALESCE(jsonb_agg(row_json ORDER BY created_at DESC), '[]') INTO v_bespoke_hot_rows
  FROM (
  ...
  INSERT INTO tenant_dashboard_stats (
    ...
    repairs_hot_rows, bespoke_hot_rows,
    ...
    v_repairs_hot_rows, v_bespoke_hot_rows,
    now(), NULL
  )
  ON CONFLICT (tenant_id) DO UPDATE SET
    ...
    repairs_hot_rows = EXCLUDED.repairs_hot_rows,
    bespoke_hot_rows = EXCLUDED.bespoke_hot_rows,
    computed_at = now(),
...
EXCEPTION WHEN OTHERS THEN
  ...
  ON CONFLICT (tenant_id) DO UPDATE SET last_refresh_error = EXCLUDED.last_refresh_error;
  RAISE NOTICE 'refresh_tenant_dashboard_stats(%) failed: %', p_tenant_id, SQLERRM;
END;
$$;

COMMENT ON COLUMN tenant_dashboard_stats.repairs_hot_rows IS
  'jsonb array of the 200 most recent repairs with all list-UI columns + embedded customer. Refreshed alongside the rest of the stats row.';
COMMENT ON COLUMN tenant_dashboard_stats.bespoke_hot_rows IS
  'jsonb array of the 200 most recent bespoke jobs with all list-UI columns + embedded customer. Refreshed alongside the rest of the stats row.';
