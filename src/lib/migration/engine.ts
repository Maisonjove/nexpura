...
export const INVENTORY_DEFAULT_MAPPINGS: PatternMap[] = [
  ...
  { sourcePatterns: ['category', 'type', 'product type', 'item type', 'classification'], dest: 'category' },
  ...
  try {
    ...
    const { data: created, error } = await admin
      ...
      .insert({
        ...
        cost_price: parseFloat(String(mappedData.cost_price || mappedData.cost || '0')) || 0,
