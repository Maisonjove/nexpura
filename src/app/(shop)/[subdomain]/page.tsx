...
export default async function ShopHomePage({ params }: Props) {
  ...
  // Fetch featured inventory (active items)
  const { data: items } = await supabase
    .from("inventory")
    .select("id, name, images, metal, stone, retail_price, category, description")
        ...
        >
          <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
              <Link href={`/shop/${subdomain}/catalogue`} className="text-white/80 hover:text-white text-sm transition-colors">
                Catalogue
              </Link>
          ...
          <section className="py-16 px-4">
            <div className="max-w-6xl mx-auto">
                ...
                {items?.map((item) => (
                  ...
                  >
                    ...
                    <div className="p-3">
                      ...
                      {item.metal && (
                        <p className="text-xs text-stone-500 truncate mt-0.5">{item.metal}{item.stone ? ` · ${item.stone}` : ""}</p>
                      )}
              ...
              <div className="text-center mt-10">
                <Link
                  href={`/shop/${subdomain}/catalogue`}
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-medium border-2 transition-colors hover:text-white"
