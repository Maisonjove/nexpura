...
export default function PrintTagModal({ item, tenantName, onClose }: PrintTagModalProps) {
  ...
  useEffect(() => {
    getStockTagTemplates().then((data) => {
      setTemplates(data)
      const defaultTpl = data.find((t: StockTagTemplate) => (t as StockTagTemplate & { is_default?: boolean }).is_default) ?? data[0]
      ...
      <html>
        <head>
          ...
          <style>
            ...
            @media print {
              body { padding: 0; }
              .print-tag { page-break-after: always; }
            }
          </style>
        </head>
        <body>
          <div class="print-tag">${printContents}</div>
