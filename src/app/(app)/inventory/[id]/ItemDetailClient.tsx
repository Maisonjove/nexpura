...
function SpecRow({ label, value }: { label: string; value: string | number | null | undefined }) {
  ...
  return (
    <div className="flex justify-between py-2 border-b border-platinum/50 last:border-0">
      <span className="text-sm text-forest/50">{label}</span>
      <span className="text-sm font-medium text-forest capitalize">{String(value)}</span>
    </div>
  );
}
    ...
    <>
      ...
      {showArchiveConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl border border-platinum shadow-xl w-full max-w-sm p-6">
            <h3 className="font-fraunces text-lg font-semibold text-forest mb-2">Archive Item?</h3>
            <p className="text-sm text-forest/60 mb-6">This item will be soft-deleted and hidden from your inventory. This cannot be easily undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowArchiveConfirm(false)} className="flex-1 py-2.5 text-sm font-medium text-forest border border-forest rounded-lg hover:bg-forest hover:text-white transition-colors">
                Cancel
              </button>
              <button
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-forest/50">
          ...
          <span className="text-forest truncate max-w-[200px]">{item.name}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-platinum p-6">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  <h1 className="font-fraunces text-2xl font-semibold text-forest">{item.name}</h1>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {item.sku && (
                      <span className="text-xs font-mono bg-ivory border border-platinum px-2 py-0.5 rounded text-forest/60">
                        {item.sku}
                      </span>
                    )}
                    {item.stock_categories && (
                      <span className="text-xs bg-sage/10 text-sage px-2 py-0.5 rounded">
                        {item.stock_categories.name}
                      </span>
                    )}
                    <span className="text-xs bg-ivory border border-platinum text-forest/50 px-2 py-0.5 rounded capitalize">
                    ...
                    {item.is_featured && (
                      <span className="text-xs bg-gold/10 text-gold border border-gold/20 px-2 py-0.5 rounded flex items-center gap-1">
              ...
              {item.description && (
                <p className="mt-4 text-sm text-forest/60 border-t border-platinum pt-4">{item.description}</p>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl border border-platinum p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                <div>
                  <p className="text-xs font-medium text-forest/40 uppercase tracking-wider mb-2">Metal</p>
                ...
                <div>
                  <p className="text-xs font-medium text-forest/40 uppercase tracking-wider mb-2">Stone</p>
                ...
                {(item.ring_size || item.dimensions || item.barcode || item.jewellery_type) && (
                  <div className="sm:col-span-2 mt-4">
                    <p className="text-xs font-medium text-forest/40 uppercase tracking-wider mb-2">Other</p>
                ...
                {(item.supplier_name || item.supplier_sku) && (
                  <div className="sm:col-span-2 mt-4">
                    <p className="text-xs font-medium text-forest/40 uppercase tracking-wider mb-2">Supplier</p>
            ...
            <div className="bg-white rounded-xl border border-platinum p-6">
              <h2 className="font-fraunces text-lg font-semibold text-forest mb-4">Stock Movement History</h2>
              {movements.length === 0 ? (
                <p className="text-sm text-forest/40 text-center py-6">No movements recorded yet</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-platinum">
                        <th className="text-left pb-3 text-xs font-medium text-forest/50 uppercase tracking-wider">Date</th>
                        <th className="text-left pb-3 text-xs font-medium text-forest/50 uppercase tracking-wider">Type</th>
                        <th className="text-right pb-3 text-xs font-medium text-forest/50 uppercase tracking-wider">Change</th>
                        <th className="text-right pb-3 text-xs font-medium text-forest/50 uppercase tracking-wider">Qty After</th>
                        <th className="text-left pb-3 text-xs font-medium text-forest/50 uppercase tracking-wider">Notes</th>
                        <th className="text-left pb-3 text-xs font-medium text-forest/50 uppercase tracking-wider">By</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-platinum/50">
                      {movements.map((m) => (
                        <tr key={m.id} className="hover:bg-ivory/30 transition-colors">
                          <td className="py-3 text-forest/50 whitespace-nowrap">
                          ...
                          <td className="py-3 text-right text-forest font-medium">{m.quantity_after}</td>
                          <td className="py-3 text-forest/50 max-w-[200px] truncate">{m.notes || "—"}</td>
                          <td className="py-3 text-forest/40 whitespace-nowrap">
                            {m.users?.full_name || "—"}
                          </td>
                        </tr>
                      ))}
          ...
          <div className="space-y-4">
            {/* Stock level */}
            <div className="bg-white rounded-xl border border-platinum p-6">
              <p className="text-xs font-medium text-forest/50 uppercase tracking-wider mb-3">Current Stock</p>
              <p className={`font-fraunces text-5xl font-semibold ${isLowStock && item.track_quantity ? "text-red-500" : "text-forest"}`}>
                {item.quantity}
              </p>
              <p className="text-sm text-forest/40 mt-1">units in stock</p>
              ...
              {!item.track_quantity && (
                <p className="text-xs text-forest/30 mt-2">Quantity tracking disabled</p>
              )}

              <button
                onClick={() => setShowAdjustModal(true)}
                className="w-full mt-4 py-2.5 bg-sage text-white text-sm font-medium rounded-lg hover:bg-sage/90 transition-colors"
              >
                Adjust Stock
              </button>
            </div>
            ...
            {/* Pricing */}
            <div className="bg-white rounded-xl border border-platinum p-6">
              <p className="text-xs font-medium text-forest/50 uppercase tracking-wider mb-4">Pricing</p>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-forest/50">Cost</span>
                  <span className="text-sm font-medium text-forest">
                    {item.cost_price != null ? `£${item.cost_price.toFixed(2)}` : "—"}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-forest/50">Wholesale</span>
                  <span className="text-sm font-medium text-forest">
                    {item.wholesale_price != null ? `£${item.wholesale_price.toFixed(2)}` : "—"}
                  </span>
                </div>
                <div className="flex justify-between items-center border-t border-platinum pt-3">
                  <span className="text-sm font-medium text-forest">Retail</span>
                ...
                {margin !== null && (
                  <div className={`flex justify-between items-center px-3 py-2 rounded-lg ${
                    ...
                    <span className="text-sm text-forest/60">Margin</span>
