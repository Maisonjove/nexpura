...
export default async function InvoicesPage({
  ...
  // Whole invoices payload (rows + count + headline stats) cached behind
  // unstable_cache + the per-tenant `invoices:{tenantId}` tag. Every
  // mutation on invoices (create, update, payment, void) calls
  // revalidateTag from invoices/actions.ts + sales/actions.ts (sales
  // create invoices) so new/changed invoices appear on next nav.
  const fetchInvoicesPayload = unstable_cache(
    async () => {
      const [outstandingRes, overdueRes, paidThisMonthRes, listResult, countResult] = await Promise.all([
        admin
          .from("invoices").select("amount_due")
