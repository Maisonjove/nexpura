...
import BillingClientLegacy from "../../../billing/BillingClientLegacy";
import { getEntitlementContext } from "@/lib/auth/entitlements";
import { SUPPORTED_CURRENCIES, type CurrencyCode } from "@/data/pricing";

export const metadata = { title: "Billing (Before) — Feedback Review" };

export default async function BillingBeforePage() {
  const auth = await getAuthContext();
  if (!auth) redirect("/login");

  const { tenantId, email, permissions } = auth;
