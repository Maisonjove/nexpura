...
/**
 * The 8-card category grid + drill-down view. Extracted from
 * DashboardClient into its own chunk so the main dashboard bundle —
 * which only needs to render the header, summary strip, attention
 * list, and sidebar — hydrates faster. The grid still server-renders
 * (next/dynamic ssr:true) so there's no layout shift; only the client
 * JS for this subtree is split into a secondary chunk that loads in
 * parallel with the main bundle.
 *
 * All SECTIONS data, icon SVGs, ActionCard helper, and ModuleDataPanel
 * lazy-loader live in this file. None of that weight ships in the
 * parent client bundle.
 */

// Drill-down data panels only render when the user clicks a category card —
// never on first paint. Split them into a lazy chunk so their ~280 lines of
// JSX + the WorkshopPanel tab state machine stay off the cold initial bundle.
const ModuleDataPanel = dynamic(() => import("./DashboardPanels"), {
  ssr: false,
  loading: () => (
    <div className="mb-6 bg-[#FAFAF8] border border-[#E8E4DF] rounded-xl overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.04)] animate-pulse">
...
// ─── Types (mirrored locally so this chunk is self-contained) ──────────────
...
type RecentSale = { id: string; saleNumber: string; customer: string | null };

// ─── Icons (scoped to this chunk — not shared with main dashboard bundle) ──
const icons = {
  plus: (
...
interface DashboardCategoryGridProps {
  bp: string;
...
export default function DashboardCategoryGrid({
  bp,
  salesThisMonthCount,
  activeRepairsCount,
  activeJobsCount,
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      items: [
        ...
        { title: "New Appraisal", description: "Create an appraisal", icon: icons.plus, href: `${bp}/appraisals/new` },
    ...
    },
    {
      ...
      items: [
        ...
        { title: "New Task", description: "Create a task", icon: icons.plus, href: `${bp}/tasks/new` },
