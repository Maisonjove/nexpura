...
export default function CustomerDetailClient({ customer }: { customer: Customer }) {
  ...
  const infoItem = (label: string, value: string | null | undefined) => (
    <div key={label}>
      <dt className="text-xs font-medium text-forest/50 uppercase tracking-wide mb-0.5">{label}</dt>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/customers"
          className="flex items-center gap-2 text-sm text-forest/50 hover:text-forest transition-colors"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="px-4 py-2 text-sm font-medium bg-sage text-white rounded-lg hover:bg-sage/90 transition-colors"
          >
            Edit Customer
          </Link>
        </div>
      ...
      <div className="bg-white rounded-xl border border-platinum p-6">
        <div className="flex items-start gap-4">
          <div className="w-16 h-16 rounded-2xl bg-sage/15 flex items-center justify-center flex-shrink-0">
            <span className="font-fraunces text-2xl font-semibold text-sage">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {customer.is_vip && (
                <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-gold/10 text-gold border border-gold/30 uppercase tracking-wide">
                  ✦ VIP
                </span>
              )}
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-1 mt-2">
              {customer.email && (
                <a href={`mailto:${customer.email}`} className="text-sm text-sage hover:underline">
                  {customer.email}
                </a>
              )}
              {customer.mobile && (
                <a href={`tel:${customer.mobile}`} className="text-sm text-forest/70 hover:text-forest">
                  {customer.mobile}
                </a>
              )}
              {customer.phone && !customer.mobile && (
                <a href={`tel:${customer.phone}`} className="text-sm text-forest/70 hover:text-forest">
                  {customer.phone}
                </a>
              )}
      ...
      {activeTab === "Overview" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-platinum p-5">
            <h3 className="font-fraunces text-sm font-semibold text-forest mb-4 flex items-center gap-2">
      ...
      {activeTab === "Notes" && (
        <div className="space-y-4">
          {/* Add note form */}
          <div className="bg-white rounded-xl border border-platinum p-5">
            <h3 className="font-fraunces text-sm font-semibold text-forest mb-3">Add Note</h3>
          ...
          {notes ? (
              ...
              {notes.split("\n\n").reverse().map((note, i) => {
                ...
                return (
                  <div key={i} className="bg-white rounded-xl border border-platinum p-4">
                    {match ? (
                      <>
                        <p className="text-xs text-forest/40 mb-1.5">{match[1]}</p>
                        <p className="text-sm text-forest whitespace-pre-wrap">{match[2]}</p>
                      </>
                    ) : (
                      <p className="text-sm text-forest whitespace-pre-wrap">{note}</p>
                    )}
                  </div>
                );
              })}
            ...
            <div className="bg-white rounded-xl border border-platinum p-10 text-center">
              <p className="text-forest/40 text-sm">No notes yet. Add one above.</p>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === "Activity" && (
        <div className="bg-white rounded-xl border border-platinum p-10 text-center">
          <div className="w-12 h-12 mx-auto mb-3 rounded-xl bg-sage/10 flex items-center justify-center">
            <svg className="w-6 h-6 text-sage" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
          </div>
          <p className="font-fraunces text-base font-semibold text-forest">No activity yet</p>
          <p className="text-sm text-forest/50 mt-1">Jobs, repairs and invoices will appear here</p>
        </div>
      )}
      ...
      {showArchiveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-platinum shadow-2xl p-6 max-w-sm w-full">
            <div className="w-12 h-12 rounded-xl bg-red-50 flex items-center justify-center mb-4">
              <svg className="w-6 h-6 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
            </div>
            <h3 className="font-fraunces text-lg font-semibold text-forest mb-2">Archive Customer</h3>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2.5 text-sm font-medium border border-forest/20 text-forest rounded-lg hover:border-forest/40 transition-colors"
              >
                Cancel
              </button>
              <button
