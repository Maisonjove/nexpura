...
/**
 * Per-entity audit history. Reuses the AuditDiffView component from
 * /settings/activity but scoped to a single customer — every edit on
 * this row, in chronological order, with the full diff inline.
 *
 * Auth: tenant-scoped. Any authed user inside the tenant can view
 * their own customer's history (same gate as /customers/[id]).
 */

export const metadata = { title: "Customer history — Nexpura" };
...
export default async function CustomerHistoryPage({
  ...
  // Tenant scope on the customer row first — this is also what
  // gates "did I have permission to see this customer at all".
  ...
  if (!customer) {
    return (
      <div className="max-w-3xl mx-auto p-6">
        <h1 className="text-xl font-semibold text-stone-900">Customer not found</h1>
        <p className="text-sm text-stone-500 mt-2">
          This customer doesn&apos;t exist or you don&apos;t have permission to view it.
        </p>
        <Link href="/customers" className="text-sm text-stone-700 underline mt-4 inline-block">
          ← Back to customers
        </Link>
      </div>
    );
  ...
  // Audit-log entries are tenant-scoped — defence-in-depth re-filter
  // by tenant + entity_id even though the customer scope above
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-stone-500 text-sm mt-1">
          Every recorded change on this customer record. Most recent first.
        </p>
      </div>

      {safeLogs.length === 0 ? (
        <div className="border border-stone-200 rounded-2xl p-10 text-center text-stone-500">
          No edits recorded for this customer yet.
        </div>
      ) : (
