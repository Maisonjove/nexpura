  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="font-fraunces text-2xl font-semibold text-forest">Communications</h1>
        <Link
      ...
      {comms.length === 0 ? (
        <div className="bg-white border border-platinum rounded-xl p-16 text-center">
          ...
          <h3 className="font-fraunces text-lg font-semibold text-forest">No messages yet</h3>
          <p className="text-forest/50 mt-1 text-sm">Send your first message or note to a customer.</p>
          <Link
            href="/communications/new"
        ...
        <div className="bg-white border border-platinum rounded-xl overflow-hidden shadow-sm divide-y divide-platinum">
          {comms.map((comm) => (
            <div key={comm.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-ivory/50 transition-colors">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                <span className="text-xs font-medium capitalize text-forest/50">{comm.type}</span>
                <span
                  className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${
                    STATUS_COLOURS[comm.status] || "bg-forest/10 text-forest/60"
                  }`}
                >
                  {comm.status}
                </span>
