...
function FinancialReportTab({ gstRate }: { gstRate: number }) {
  ...
  const buildPrintHTML = (r: ReportData, gst: number) => {
    ...
    return `<!DOCTYPE html><html><head><title>Financial Report</title><style>
      ...
      <table>
        ${row('Total Revenue', fmtFull(r.totalRevenue), true)}
        ${row('Total Refunds', '−' + fmtFull(r.totalRefunds))}
        ${row('Net Revenue', fmtFull(r.netRevenue), true)}
        ${row('GST Collected (est.)', fmtFull(r.gstCollected))}
        ${row('Total Transactions', String(r.totalTransactions))}
        ${row('Average Sale Value', fmtFull(r.avgSaleValue))}
      </table>
      <h2>Revenue by Category</h2>
      <table>
        ${row('POS Sales', fmtFull(r.revenueByCategory.posSales))}
        ${row('Invoices', fmtFull(r.revenueByCategory.invoices))}
        ${row('Repairs', fmtFull(r.revenueByCategory.repairs))}
        ${row('Bespoke Jobs', fmtFull(r.revenueByCategory.bespoke))}
      </table>
      <h2>Payment Methods</h2>
      <table>${Object.entries(r.paymentBreakdown).map(([m, v]) => row(m.charAt(0).toUpperCase() + m.slice(1), fmtFull(v as number))).join('') || '<tr><td colspan="2" style="padding:8px 12px;color:#9ca3af">No data</td></tr>'}</table>
      <h2>Top 5 Customers</h2>
      <table>${r.topCustomers.map((c, i) => row(`${i + 1}. ${c.name}`, fmtFull(c.total))).join('') || '<tr><td colspan="2" style="padding:8px 12px;color:#9ca3af">No data</td></tr>'}</table>
      <h2>Top 5 Products</h2>
      <table>${r.topProducts.map((p, i) => row(`${i + 1}. ${p.name} (${p.qty} units)`, fmtFull(p.revenue))).join('') || '<tr><td colspan="2" style="padding:8px 12px;color:#9ca3af">No data</td></tr>'}</table>
    ...
    <div className="space-y-6">
      ...
      {report && !loadingReport && (
        <>
          {/* Key metrics */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label="Total Revenue" value={fmt(report.totalRevenue)} icon={<DollarSign size={16} />} />
            <StatCard label="Net Revenue" value={fmt(report.netRevenue)} sub={`After ${fmt(report.totalRefunds)} refunds`} icon={<TrendingUp size={16} />} />
            <StatCard label="GST Collected" value={fmt(report.gstCollected)} sub="Estimate" icon={<ReceiptText size={16} />} />
            <StatCard label="Transactions" value={String(report.totalTransactions)} sub={`Avg ${fmt(report.avgSaleValue)}`} icon={<CreditCard size={16} />} />
          </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {[
                ...
                <div key={cat.label} className="bg-stone-50 rounded-lg p-4">
                  ...
                  <p className="text-lg font-semibold text-stone-900">{fmt(cat.value)}</p>
                ...
                <div className="divide-y divide-stone-100">
                  {report.topCustomers.map((c, i) => (
                    <div key={i} className="px-5 py-3 flex items-center justify-between">
                      ...
                      <span className="text-sm font-semibold text-stone-900">{fmt(c.total)}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="divide-y divide-stone-100">
                  {report.topProducts.map((p, i) => (
                    <div key={i} className="px-5 py-3 flex items-center justify-between">
                      ...
                      <span className="text-sm font-semibold text-stone-900">{fmt(p.revenue)}</span>
                    </div>
                  ))}
                </div>
              )}
...
export default function FinancialsClient({
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
          ...
          <section>
            ...
            {loadingMetrics ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <StatCard
                  label="GST Collected"
                  value={fmt(metrics.gstCollected)}
                ...
                <StatCard
                  ...
                  value={fmt(metrics.refundsThisMonth)}
                  sub={`${metrics.refundCount} refund${metrics.refundCount !== 1 ? 's' : ''}`}
                  icon={<TrendingDown size={16} />}
                />
                <StatCard
                  label="Avg Sale Value"
                  value={fmt(metrics.avgSaleValue)}
                ...
                <>
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="text-center p-4 bg-stone-50 rounded-lg">
                      ...
                      <p className="text-xl font-semibold text-stone-900">{fmt(metrics.gstCollected)}</p>
                    ...
                    <div className="text-center p-4 bg-emerald-50 rounded-lg">
                      ...
                      <p className="text-xl font-semibold text-emerald-700">{fmt(metrics.gstCollected)}</p>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        {metrics.quarterlyGST.map((q) => (
                          <div key={q.label} className="bg-stone-50 rounded-lg p-3">
                            ...
                            <p className="text-base font-semibold text-stone-900">{fmt(q.gst)}</p>
                            <p className="text-xs text-stone-400">of {fmt(q.revenue)} rev.</p>
                          </div>
                        ))}
                      </div>
                    </>
      ...
      {/* Reports Tab */}
      {tab === 'reports' && <FinancialReportTab gstRate={gstRate} />}
    </div>
  );
}
