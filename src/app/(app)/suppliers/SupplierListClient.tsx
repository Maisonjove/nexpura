        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {suppliers.map((supplier) => (
            <Link
              ...
              className="bg-white border border-stone-200 rounded-xl p-5 hover:border-amber-600/40 hover:shadow-sm transition-all group"
            >
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="w-10 h-10 rounded-xl bg-stone-100 flex items-center justify-center flex-shrink-0">
                  <span className="font-semibold text-sm font-semibold text-amber-700">
                    {supplier.name[0].toUpperCase()}
                  </span>
                </div>
                <svg className="w-4 h-4 text-stone-300 group-hover:text-amber-700 transition-colors mt-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
