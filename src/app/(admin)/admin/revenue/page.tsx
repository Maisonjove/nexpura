...
function fmtDate(d: string) {
    return new Date(d).toLocaleDateString("en-AU", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" });
}
...
export default async function RevenueAdminPage() {
    let subs: {
          ...
          created_at: string;
          tenants: { name: string; is_free_forever: boolean | null } | null;
    }[] | null = null;
  ...
  try {
      ...
      const subsRes = await admin
          .from("subscriptions")
          .select("tenant_id, plan, status, trial_ends_at, current_period_end, created_at, tenants(name, is_free_forever)");
        ...
        <div className="max-w-5xl mx-auto py-10 px-4">
          {/* Header */}
              <div className="flex items-center justify-between mb-8">
                      <div>
                                ...
                                <h1 className="text-2xl font-semibold text-stone-900">Revenue Overview</h1>h1>
                                <p className="text-sm text-stone-500 mt-0.5">Live MRR, ARR, plan breakdown, and subscription metrics</p>p>
                      </div>div>
              </div>div>
          ...
          { label: "New (30d)", value: newTenants, icon: "🆕", sub: "New tenants" },
                              ...
                              <div key={k.label} className="bg-white rounded-xl border border-stone-200 p-5">
                                          <div className="text-2xl mb-2">{k.icon}</div>div>
                                          <div className="text-2xl font-bold text-stone-900">{k.value}</div>div>
                                          <div className="text-xs text-stone-500 mt-0.5">{k.sub}</div>div>
                              </div>div>
                            ))}
              </div>div>
          ...
          { label: "Churn Rate", value: `${churnRate}%`, color: "text-stone-900" },
                              ...
                              <div key={k.label} className="bg-white rounded-xl border border-stone-200 p-4">
                                          <div className={`text-xl font-bold ${k.color}`}>{k.value}</div>div>
                                          <div className="text-xs text-stone-500 mt-0.5">{k.label}</div>div>
                              </div>div>
                            ))}
              </div>div>
          ...
          {/* Plan breakdown */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                      <div className="bg-white rounded-xl border border-stone-200 p-5">
                                <h2 className="text-sm font-semibold text-stone-700 mb-4">Plan Distribution (Active)</h2>h2>
                        ...
                        return (
                                          <div key={plan}>
                                                            <div className="flex justify-between text-sm mb-1">
                                                                                <span className="font-medium text-stone-900 capitalize">{plan}</span>span>
                                                                                <span className="text-stone-500">{count} tenants · {fmtCurrency(revenue)}/mo</span>span>
                                                            </div>div>
                ...
                {/* Conversion funnel */}
                      <div className="bg-white rounded-xl border border-stone-200 p-5">
                                <h2 className="text-sm font-semibold text-stone-700 mb-4">Conversion Funnel</h2>h2>
                                <div className="space-y-3">
                                  {[
          ...
          { label: "Conversion Rate", value: `${conversionRate}%`, width: parseFloat(conversionRate) },
                                      ...
                                      <div key={row.label}>
                                                      <div className="flex justify-between text-sm mb-1">
                                                                        <span className="text-stone-600">{row.label}</span>span>
                                                                        <span className="font-medium text-stone-900">{row.value}</span>span>
                                                      </div>div>
                                                      <div className="h-1.5 bg-stone-100 rounded-full overflow-hidden">
                                                                        <div className="h-full bg-amber-700/60 rounded-full" style={{ width: `${row.width}%` }} />
                                                      </div>div>
                                      </div>div>
                                    ))}
                                </div>div>
          ...
          {/* Active subscriptions table */}
              <div className="bg-white rounded-xl border border-stone-200 overflow-hidden mb-6">
                      <div className="px-5 py-4 border-b border-stone-200 flex items-center justify-between">
                                <h2 className="text-sm font-semibold text-stone-700">Active Subscriptions</h2>h2>
                                <span className="text-xs text-stone-400">{activeSubs.length} tenants · {fmtCurrency(mrr)} MRR</span>span>
                      </div>div>
                      <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                            <thead className="bg-stone-50">
                                                          <tr>
                                                                          <th className="text-left px-5 py-3 text-xs font-medium text-stone-500">Tenant</th>th>
                                                                          <th className="text-left px-4 py-3 text-xs font-medium text-stone-500">Plan</th>th>
                                                                          <th className="text-right px-4 py-3 text-xs font-medium text-stone-500">MRR</th>th>
                                                                          <th className="text-left px-4 py-3 text-xs font-medium text-stone-500">Renews</th>th>
                                                          </tr>tr>
                                            </thead>thead>
                          ...
                          <tr key={sub.tenant_id} className="hover:bg-stone-50">
                                            <td className="px-5 py-3">
                                              ...
                                              {sub.tenants?.is_free_forever && (
                                                  <span className="ml-2 text-xs text-green-600 bg-green-50 px-1.5 py-0.5 rounded">Free</span>span>
                                                                )}
                                            </td>td>
                                            <td className="px-4 py-3">
                                                                <span className="capitalize text-stone-700">{sub.plan}</span>span>
                                            </td>td>
                                            <td className="px-4 py-3 text-right font-medium text-stone-900">
                                              {sub.tenants?.is_free_forever ? "—" : fmtCurrency(PLAN_PRICES[sub.plan] ?? 0)}
                                            </td>td>
          ...
          {pastDueSubs.length > 0 && (
                  <div className="bg-red-50 border border-red-200 rounded-xl p-5">
                            <h2 className="text-sm font-semibold text-red-800 mb-3">⚠️ At Risk — Payment Required ({pastDueSubs.length})</h2>h2>
                            <div className="space-y-2">
                              {pastDueSubs.map((sub) => (
                                  <div key={sub.tenant_id} className="flex items-center justify-between">
                                                  ...
                                                  <span className="text-xs text-red-600 capitalize">{sub.plan}</span>span>
                                  </div>div>
                                ))}
                            </div>div>
                  </div>div>
