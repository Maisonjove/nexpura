...
export default async function PrintInvoicePage({
  ...
  const printedAt = new Date().toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" });
      ...
      <head>
        ...
        <style dangerouslySetInnerHTML={{__html: `
          ...
          * { box-sizing: border-box; }
          ...
          .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20pt; border-bottom: 2px solid #000; padding-bottom: 12pt; }
          ...
          .total-section table { font-size: 10pt; }
          .total-section td, .total-section th { border: none; padding: 3px 8px; }
          ...
          .status-badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 9pt; font-weight: bold; text-transform: uppercase; background: ${invoice.status === "paid" ? "#1c1917" : invoice.status === "partial" ? "#fef3c7" : "#f5f5f4"}; color: ${invoice.status === "paid" ? "#fff" : invoice.status === "partial" ? "#92400e" : "#666"}; }
        `}} />
      </head>
      <body>
        <button
